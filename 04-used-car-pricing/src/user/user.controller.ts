...
export class UserController {
    ...
    @Get()
    async getSomeUsers(@Query('username') username: string, @Query('email') email: string) {
        let users: User[] = [], fieldname: string = '';
        ...
        if(!users?.length)
            throw new NotFoundException(`No user has been found with such ${fieldname}!`);
